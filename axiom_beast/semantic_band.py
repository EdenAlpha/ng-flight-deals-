...
if __name__=='__main__':
 ...
 for path in sys.argv[1:]:
  z=zipfile.ZipFile(path);members=[z.read(i) for i in z.infolist() if not i.is_dir()];lens=list(map(len,members));t=time.time();p=pack(members);u=unpack(p,lens);print(os.path.basename(path),'raw',sum(lens),'pack',len(p),'ok',u==members,'sec',time.time()-t)
