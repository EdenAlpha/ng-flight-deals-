...
def split_line(line,delim=44):
    ...
    while i<len(line):
        ...
        if c==delim and not inq:
            fs.append(line[start:i]);start=i+1
        i+=1
    if inq:return None
...
def _unpack_raw(rep,n):
    ...
    if p!=len(rep):raise ValueError('raw col trailing')
    return out
...
def _pack_int(vals):
    ...
    absb=bytearray();deltab=bytearray();prev=0
    for q in parsed:
        if q is None:continue
        absb+=enc_svar(q);deltab+=enc_svar(q-prev);prev=q
    mode=0 if len(zlib.compress(absb,6))<=len(zlib.compress(deltab,6)) else 1
    x=absb if mode==0 else deltab
    return b'I'+bytes([mode])+vi(len(bits))+bits+x
...
def _pack_group_int(vals,cols,keyids):
    parsed=[];bits=bytearray((len(vals)+7)//8);state={};stream=bytearray()
    for i,v in enumerate(vals):
        if v==b'':parsed.append(None);continue
        if not INT.fullmatch(v):return None
        q=int(v)
        if str(q).encode()!=v:return None
        bits[i>>3]|=1<<(i&7);key=tuple(cols[k][i] for k in keyids);prev=state.get(key,0);stream+=enc_svar(q-prev);state[key]=q;parsed.append(q)
...
def _choose(vals):
    cand=[_pack_raw(vals),_pack_dict(vals)]
    a=_pack_int(vals); b=_pack_date(vals)
    if a:cand.append(a)
    if b:cand.append(b)
    return min(cand,key=lambda x:len(zlib.compress(x,6)))

def _sample_group_score(vals,cols,ks,limit=12000):
    state={};stream=bytearray();seen=0
    step=max(1,len(vals)//limit)
    for i in range(0,len(vals),step):
        v=vals[i]
        if v==b'':continue
        if not INT.fullmatch(v):return None
        q=int(v);key=tuple(cols[k][i] for k in ks);prev=state.get(key,0);stream+=enc_svar(q-prev);state[key]=q;seen+=1
        if seen>=limit:break
    if seen<32:return None
    return len(zlib.compress(stream,1)),len(state)

def _sample_fd_ok(vals,cols,ks,limit=12000):
    mp={};seen=0;step=max(1,len(vals)//limit)
    for i in range(0,len(vals),step):
        k=tuple(cols[x][i] for x in ks);v=vals[i]
        if k in mp and mp[k]!=v:return False
        mp[k]=v;seen+=1
        if seen>=limit:break
    return True
...
def pack(data):
    ...
    header=rows[0];body=rows[1:];nr=len(body)
    cols=[[r[j] for r in body] for j in range(nc)]
    reps=[]
    for j,c in enumerate(cols):
        cand=[_choose(c)];ksall=_keysets(j)
        fdc=[]
        for ks in ksall:
            if not _sample_fd_ok(c,cols,ks):continue
            fdc.append(ks)
        for ks in fdc[:6]:
            q=_pack_fd(c,cols,ks)
            if q:cand.append(q)
        if all(v==b'' or INT.fullmatch(v) for v in c):
            scored=[]
            for ks in ksall:
                q=_sample_group_score(c,cols,ks)
                if q is not None:scored.append((q[0],len(ks),ks))
            scored.sort()
            for _,_,ks in scored[:4]:
                q=_pack_group_int(c,cols,ks)
                if q:cand.append(q)
                q=_pack_group_int_clustered(c,cols,ks)
                if q:cand.append(q)
        reps.append(min(cand,key=lambda x:len(zlib.compress(x,6))))
    o=bytearray(MAGIC)+vi(nc)+vi(nr)
    o+=b''.join(vi(len(x))+x for x in header)
    o+=vi(len(ends))+bytes(ends)
...
def unpack(rep):
    ...
    ne,p=uv(rep,p);ends=list(rep[p:p+ne]);p+=ne
    cols=[]
    for _ in range(nc):
        l,p=uv(rep,p);r=rep[p:p+l];p+=l
        t=r[:1]
