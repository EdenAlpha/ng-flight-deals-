...
def _run(args,d):
    return subprocess.run(args,input=d,stdout=subprocess.PIPE,stderr=subprocess.DEVNULL,check=True).stdout

def zc(d,level=22):
    return _run(['zstd',f'-{level}','--ultra','-q','-c'],d)
def zd(d):
    return _run(['zstd','-d','-q','-c'],d)
def bc(d):
...
def bestc(d, allow_brotli=True):
    ...
    if allow_brotli and (shutil.which('brotli') or _pybrotli is not None):
        try: cand.append((3,bc(d)))
        except Exception: pass
    return min(cand,key=lambda x:len(x[1]))

def fast_score(d):
    # Cheap proxy for choosing transform variants, never used as final coding result.
    return len(zlib.compress(d,6))
