...
def make_zip_candidate(d):
 ...
 meta=meta_encode(entries,laws)
 raw=b''.join(members);simple=putstream(raw)
...
def compress(src,dst):
 ...
 out=MAGIC+vi(len(d))+digest+best
...
def decompress(src,dst):
 ...
 if mode==0:
  ...
  sk,p=getstream(b,p);meta,p=getstream(b,p);entries,laws=meta_decode(meta);sm=b[p];p+=1
  lens=[e[2] for e in entries]
  if sm==0:
