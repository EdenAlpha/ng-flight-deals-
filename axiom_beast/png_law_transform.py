...
def pack(d):
 ...
 ih=[x for x in cs if x[1]==b'IHDR']
 ids=[x for x in cs if x[1]==b'IDAT']
 if len(ih)!=1 or not ids:return None
 _,_,ds,ln=ih[0];H=d[ds:ds+ln]
 if len(H)!=13:return None
 w=int.from_bytes(H[0:4],'big');h=int.from_bytes(H[4:8],'big');bd=H[8];ct=H[9];inter=H[12]
...
def unpack(rep):
 ...
 cs=chunks(sk);ih=[x for x in cs if x[1]==b'IHDR'][0];ids=[x for x in cs if x[1]==b'IDAT'];_,_,ds,ln=ih;H=sk[ds:ds+ln];w=int.from_bytes(H[0:4],'big');h=int.from_bytes(H[4:8],'big');bd=H[8];ct=H[9]
 raw=body
 if mode:
  bpp={0:1,2:3,3:1,4:2,6:4}[ct];raw=refilter(body,w,h,bpp)
