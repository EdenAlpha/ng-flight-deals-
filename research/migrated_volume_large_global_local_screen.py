#!/usr/bin/env python3
"""Large native scale screen for global-predict/local-entropy migrated codec."""
from __future__ import annotations
import argparse,json
from pathlib import Path
import numpy as np
import migrated_volume_large_native_screen as large
import migrated_volume_3d_global_predict_local_entropy as gp
...
def compete(X,eps):
 rows=[]
 g,gr=gp.compete(X,eps);rows.append({'name':g['name'],'bytes':int(g['bytes'])+OVERHEAD,'maxerr':float(g['maxerr']),'diag':g['diag']})
 for tid in TIDS:
...
def stream(objects,tag,eps):
 ...
 try:
  ...
  for wi,frac in enumerate(large.FRACTIONS):
   ...
   rows.append(row);print('MVGPLOCAL_LARGE',tag,wi,X.shape,sb,best['bytes'],gain,best['name'],best['diag'],flush=True)
