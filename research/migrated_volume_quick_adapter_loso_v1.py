...
def fit_base(train_tiles,seed):
    ...
    class Base(nn.Module):
        def __init__(self,d):
            super().__init__()
            self.trunk=nn.Sequential(nn.Linear(d,160),nn.SiLU(),nn.LayerNorm(160),nn.Linear(160,112),nn.SiLU(),nn.Linear(112,64),nn.SiLU())
...
def adapter_update(net,X,Y,P,B,bias,steps,lr):
    ...
    for _ in range(int(steps)):
        logits=logits_with_adapter(net,X,P,B,bias);loss=F.cross_entropy(logits,Y)
        if B.grad is not None:B.grad.zero_()
        if bias.grad is not None:bias.grad.zero_()
        loss.backward()
        with __import__('torch').no_grad():B-=lr*B.grad;bias-=lr*bias.grad
...
def score_and_adapt(net,mu,sd,static,X,eps,meta,P,B,bias,reservoir,first_tile):
    ...
    XE=torch.from_numpy(A);YY=torch.from_numpy(Y);bits=boundary_bits(static,R,I);chunk_bits=[]
    for ci,s in enumerate(range(0,len(XE),CHUNK)):
        e=min(len(XE),s+CHUNK);xx=XE[s:e];yy=YY[s:e]
        with torch.no_grad():
            lp=F.log_softmax(logits_with_adapter(net,xx,P,B,bias),1)/math.log(2);cb=float((-lp[torch.arange(len(yy)),yy]).sum())
...
def main(a):
    ...
    for ds in SURVEYS:
        ...
        for frac in sorted(set((TRAIN_FRAC,)+TEST_FRACS)):
            X,ep,md=b.extract(ds,m,e,frac);raw[ds][frac]=(crop(X),ep,md);print('RAW',ds,frac,crop(X).shape,flush=True)
    targets=[]
    for ti,target in enumerate(SURVEYS):
        train_ids=tuple(q0 for q0 in SURVEYS if q0!=target);train=[(raw[q0][TRAIN_FRAC][0],raw[q0][TRAIN_FRAC][1]) for q0 in train_ids]
        net,mu,sd,static=fit_base(train,20260819+ti);P,B,bias=new_adapter(91000+ti);reservoir=[];base_rows=[];adapt_rows=[]
        for pi,frac in enumerate(TEST_FRACS):
            ...
            row=score_and_adapt(net,mu,sd,static,X,ep,meta,P,B,bias,reservoir,pi==0);adapt_rows.append(row);print('QUICK_ADAPT',json.dumps(row),flush=True)
        base_gain=sum(z['sz3_bytes'] for z in base_rows)/sum(z['ideal_bytes_plus_header'] for z in base_rows);adapt_gain=sum(z['sz3_bytes'] for z in adapt_rows)/sum(z['ideal_bytes_plus_header'] for z in adapt_rows)
    ...
    out={'kind':'four-survey-causal-quick-adapter-loso-v1','status':'diagnostic_ideal_probability_rate_not_serialized_codec','surveys':list(SURVEYS),'held_out_survey_used_in_base_training':False,'test_positions_all_charged':True,'adapter_updates_only_after_scoring_decoded_chunks':True,'adapter_state_persists_across_heldout_positions':True,'adapter_replay_uses_only_already_decoded_samples':True,'adapter_rank':RANK,'adapter_trainable_parameters':int(RANK*b.NCLASS+b.NCLASS),'chunk':CHUNK,'adapter_lr':ADAPTER_LR,'first_chunk_steps':FIRST_CHUNK_STEPS,'later_chunk_steps':LATER_CHUNK_STEPS,'replay_cap':REPLAY_CAP,'replay_steps_between_tiles':REPLAY_STEPS_BETWEEN_TILES,'targets':targets,'base_overall_byte_weighted_gain_vs_sz3':float(sum(z['sz3_bytes'] for z in allb)/sum(z['ideal_bytes_plus_header'] for z in allb)),'quick_adapter_overall_byte_weighted_gain_vs_sz3':float(sum(z['sz3_bytes'] for z in alla)/sum(z['ideal_bytes_plus_header'] for z in alla)),'all_quick_adapter_positions_beat_sz3':bool(all(z['gain_vs_sz3_ideal']>=1 for z in alla)),'note':'Base model weights are separately trained on the other three surveys for each LOSO split. Adapter has only 95 trainable numbers and begins at zero on the held-out survey. Ideal rate only; production determinism/serialization/throughput remain future gates.'}
