...
def ibm32_to_float(raw,endian='>'):
    # Use segyio's own production conversion when available so streamed IBM
    # values are bit-identical to the reference decoder, including underflow.
    if endian=='>':
        try:
            import segyio
            words=np.frombuffer(raw,dtype=np.uint32)
            return np.asarray(segyio.tools.native(words,format=1,copy=True),dtype=np.float32)
        except Exception:
            pass
...
class SegySequential:
    def __init__(self,reader):
        ...
        # Rev0/1 normally big-endian; only accept little endian when big fields are impossible.
