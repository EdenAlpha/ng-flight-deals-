...
def fft_spatial_corr(Z,maxoff):
    # Z is channel-standardized [C,T]. Zero-pad channel dimension so the IFFT gives non-circular pair sums.
    c,t=Z.shape;nfft=1
    ...
    ac=np.fft.irfft(spec,n=nfft)[:maxoff+1].real
    den=np.array([(c-d)*t for d in range(maxoff+1)],np.float64)
    return ac/den

def offset_detail(Z,d):
    a=Z[:-d] if d else Z;b=Z[d:] if d else Z
    corr=float(np.mean(a*b));sg=float(np.mean((a>=0)==(b>=0)))
    # Coarse nonlinear dependence: 8x8 sign/magnitude bins, reported as empirical MI diagnostic.
    def bin8(x):
...
def main(path):
    with h5py.File(path,'r') as f:
        ...
        for bi,c0 in enumerate(range(0,C,BLOCK)):
            ...
            if me>128.000001 or me>eps:raise RuntimeError(('hard',bi,me,eps));Kall[c0:c0+BLOCK]=K
    ...
    fixed=[offset_detail(Z,int(d)) for d in CHECK]
    detailed=[]
    seen=set()
    for d in list(rank[:12])+list(CHECK):
        d=int(d)
        if d not in seen:detailed.append(offset_detail(Z,d));seen.add(d)
    # Check whether 128-channel partition periodicity itself is unusually strong.
    ...
    out={'global_std':std,'eps':eps,'step':STEP,'ar_order':P,'training_samples':TRAIN,'processed_samples':NT,'analysis_interval':[A0,NT],'channels':C,'block_width':BLOCK,'model_bytes':model_bytes,'maxerr':maxerr,
         ...
         'scope':'Long-range post-AR32 spatial-lattice diagnostic. The full 6912-channel cable is split exactly as the persistent codec: each 128-channel block fits/decodes one shared float32 AR32 model using only t<1024, then recursively generates exact 256-step innovations through t<8192 with <=128 reconstruction error. On the held-out t=4096..8191 innovations, every channel is standardized over time and the spatial autocorrelation spectrum for offsets 1..512 is computed by zero-padded FFT (no circular wrap). Top/fixed offsets also report sign agreement and coarse empirical MI. Purpose: detect periodic/interrogator/nonlocal channel couplings that the existing +/-8 space-time stencil could not see. Diagnostic only, no compression claim, no AI.'}
