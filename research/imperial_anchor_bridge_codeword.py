...
@njit(cache=True)
def choose_chain(lo,hi,xodd,bound,h,phi,lam,kcost,D):
    na=lo.size; maxs=2
    ...
    for i in range(na-1):
        ...
        for sb in range(nsb):
            ...
            for sa in range(nsa):
                qa=lo[i]+sa
                c=pc[sa]+lam*abs(qb-qa)
                if hasodd:
                    mid=phi+0.5*h*(qa+qb)
                    k=int(np.rint((xodd[i]-mid)/h))
                    idx=k+D
...
def legal(x,bound,h,phi):
    ...
    if int(np.max(hi-lo))>1:raise RuntimeError(('>2 states',int(np.max(hi-lo))))
    return lo,hi
    ...
    with h5py.File(path,'r') as f:
        ...
        for name,t0,c0 in specs:
            ...
            for ph in range(PHASES):
                for lam in LAMBDAS:
                    rr,R=solve_tile(X,bound,ph,lam)
                    if rr['maxerr']>eps*(1+5e-6):raise RuntimeError(('hard error',name,rr['maxerr'],eps))
                    rr.update({'tile':name,'direct_sz3_bytes':sb,'gain_vs_sz3':sb/rr['bytes'],'bps':8*rr['bytes']/X.size});rows.append(rr)
        combos=[]
        for ph in range(PHASES):
            for lam in LAMBDAS:
                rr=[r for r in rows if r['phase']==ph and r['lambda']==lam];b=sum(r['bytes'] for r in rr);s=sum(t['sz3'] for t in tiles)
                combos.append({'phase':ph,'lambda':lam,'bytes':b,'sz3_bytes':s,'gain_vs_sz3':s/b,'bps':8*b/(C*T*len(rr)),'min_tile_gain':min(r['gain_vs_sz3'] for r in rr),'median_correction_nonzero':float(np.median([r['correction_nonzero_fraction'] for r in rr])),'anchor_bytes':sum(r['anchor_bytes'] for r in rr),'correction_bytes':sum(r['correction_bytes'] for r in rr),'anchor_reps':[r['anchor_rep'] for r in rr],'correction_reps':[r['correction_rep'] for r in rr]})
        combos.sort(key=lambda r:r['bytes']);out={'std':std,'eps':eps,'bound':bound,'lattice_step':2*bound,'anchor_pattern':'even channels plus final channel; odd channels are decoder-derived midpoint plus exact correction','combos':combos,'rows':rows,'scope':'Tolerance-bridge codeword screen. Anchor reconstruction values are not compressed independently first: legal anchor lattice states are jointly selected by exact chain DP to make omitted midpoint channels legal/cheap. Only selected anchor states and odd-channel correction are serialized; final hard error verified. One fixed phase/lambda aggregate across four precommitted tiles.'}
        print(json.dumps({'best':combos[:8]},indent=2),flush=True);json.dump(out,open('imperial_anchor_bridge_codeword.json','w'),indent=2)
