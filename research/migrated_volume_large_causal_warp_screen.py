...
The warp candidates compete on the identical large blocks against the strongest
existing monolithic v4/context/Brotli family and matched whole-block SZ3.  Every
candidate is decoder-validated and fully charged.  We separately report whether
warp prediction reduces residual activity even when it does not yet win bytes.
"""
...
def screen_stream(objects,tag,eps):
    ...
    try:
        ...
        for wi,frac in enumerate(large.FRACTIONS):
            ...
            oldbest,oldcands=base.compete(X,eps)
            oldbytes=int(oldbest['bytes'])
            wc=[]
            for tid in sorted(warp.PARAMS):
                d,b=warp.candidate(X,eps,tid);q=dict(d);q['bytes_charged']=int(len(b)+OVERHEAD);wc.append(q)
                print('WARP',tag,wi,tid,'NZ',q['nonzero_fraction'],'BYTES',q['bytes_charged'],'LAGABS',q['lag_mean_abs'],flush=True)
            wbest=min(wc,key=lambda q:(q['bytes_charged'],q['tid']))
            sb,sme=matched_sz3(X,eps)
            winner=min([{'name':oldbest['name'],'bytes':oldbytes,'kind':'incumbent'}]+[{'name':q['name'],'bytes':q['bytes_charged'],'kind':'warp','tid':q['tid']} for q in wc],key=lambda q:(q['bytes'],q['name']))
            row={
                'window_index':wi,'window_fraction':frac,'shape':list(map(int,X.shape)),'samples':int(X.size),
                ...
                'best_warp_name':wbest['name'],'best_warp_tid':int(wbest['tid']),'best_warp_bytes':int(wbest['bytes_charged']),
                ...
                'best_warp_lag_mean_abs':float(wbest['lag_mean_abs']),'best_warp_lag_nonzero_fraction':float(wbest['lag_nonzero_fraction']),
                'winner':winner,'sz3_bytes':int(sb),'sz3_maxerr':float(sme),'warp_candidates':wc,
                'incumbent_candidates':{q['name']:int(q['bytes']) for q in oldcands},
            }
            if float(sme)>eps*(1+3e-6):raise RuntimeError(('sz3 hard',tag,wi,sme,eps))
            rows.append(row);print('WARP_GATE',tag,wi,'SZ3',sb,'OLD',oldbytes,'WARP',wbest['bytes_charged'],'WG',row['best_warp_gain_vs_sz3'],'REL',row['warp_gain_vs_incumbent'],'NZ',wbest['nonzero_fraction'],'WIN',winner['name'],flush=True)
...
def run(a):
    ...
    out={
        'kind':'migrated-volume-causal-warp-large-v1','dataset_id':a.dataset,'epsilon':eps,'positions':len(rows),
        'warp_beats_incumbent':int(sum(q['warp_gain_vs_incumbent']>1 for q in rows)),
        'warp_beats_sz3':int(sum(q['best_warp_gain_vs_sz3']>1 for q in rows)),
        ...
        'median_warp_nonzero_fraction':float(np.median([q['best_warp_nonzero_fraction'] for q in rows])),
        'min_warp_nonzero_fraction':float(min(q['best_warp_nonzero_fraction'] for q in rows)),
        'all_valid':True,'zero_transmitted_warp_map':True,'streams':ss,
    }
