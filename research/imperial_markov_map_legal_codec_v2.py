...
class AE:
 ...
 def finish(self):
  self.pending+=1;self.emit(0 if self.lo<Q1 else 1);return self.w.finish()
class AD:
...
def lse(vals):
 ...
 return z+math.log(sum(math.exp(x-z) for x in vals))
...
class Markov:
 def __init__(self,seqs):
  ...
  self.cdfcache={None:np.cumsum(self.marg,dtype=np.int64)}
 def logp0(self,b):
 ...
 def cdf(self,a):
  key=None if a is None else int(a);c=self.cdfcache.get(key)
  if c is not None:return c
  w=(ALPHA*self.marg).astype(np.int64)
  for b,n in self.tr[key].items():
   i=self.idx.get(int(b))
   if i is not None:w[i]+=self.N*int(n)
  c=np.cumsum(w,dtype=np.int64);self.cdfcache[key]=c;return c
...
def map_path(x,model,bound,h,phi):
 ...
 logmap=float(np.max(mx[-1]));logmass=lse(sm[-1])
...
def arithmetic_decode(blob,nbits,n,model):
 ...
 for t in range(n):
  c=model.cdf(prev);target=int(d.target(int(c[-1])));i=int(np.searchsorted(c,target,'right'))
  if i>=len(c):raise RuntimeError(('arithmetic target',target,int(c[-1])))
  cum=0 if i==0 else int(c[i-1]);freq=int(c[i]-cum);d.take(cum,freq,int(c[-1]));q=int(model.states[i]);out[t]=q;prev=q
 return out
def fallback(x,h,phi):
 q=nearest(np.asarray(x,np.float64),h,phi);blob=Z.compress(np.ascontiguousarray(q.astype('<i4')).tobytes());qd=np.frombuffer(D.decompress(blob),dtype='<i4').astype(np.int32)
 if not np.array_equal(qd,q):raise RuntimeError('fallback replay')
 return q,blob

def encode_candidate(prev,target,eps,hf):
 bound=eps*SAFETY;h=hf*bound;_,phi,pidx=choose_phase(prev,h);buf=bytearray(struct.pack('<4sBBHH',b'MAP2',HFACTORS.index(hf),int(pidx),C,T));rows=[]
 for ci in range(C):
  ...
  if res is None:
   q,blob=fallback(x,h,phi);nbits=0;mode=1;mapbits=None;massbits=None;mls=None
  else:
   q,mapbits,massbits,mls=res;blob,nbits=arithmetic_encode(q,model);qd=arithmetic_decode(blob,nbits,T,model)
   if not np.array_equal(qd,q):raise RuntimeError(('MAP arithmetic replay',hf,ch))
   mode=0
  recon=phi+h*q;me=float(np.max(np.abs(x-recon)))
  if me>eps*(1+5e-6):raise RuntimeError(('MAP source hard',hf,ch,me,eps))
  buf.extend(struct.pack('<BII',mode,int(nbits),len(blob)));buf.extend(blob)
 ...
 for ci in range(C):
  ch=C0+ci;mode,nbits,nbytes=struct.unpack_from('<BII',raw,off);off+=9;blob=raw[off:off+nbytes];off+=nbytes;seqs=[nearest(np.asarray(d[:,ch],np.float64),h,phi) for d in prev];model=Markov(seqs)
  if mode==0:q=arithmetic_decode(blob,nbits,T,model)
  else:q=np.frombuffer(D.decompress(blob),dtype='<i4').astype(np.int32)
  if len(q)!=T:raise RuntimeError(('decoded length',len(q),T))
  RR[ci]=phi+h*q
 if off!=len(raw):raise RuntimeError(('container trailing',off,len(raw)))
 ...
 return {'h_over_eps_approx':hf,'phase_index':int(pidx),'phase':float(phi),'bytes':len(raw),'bps':8*len(raw)/(C*T),'maxerr':me,'map_channels':len(maps),'fallback_channels':C-len(maps),'mean_map_nll_bps':float(np.mean([r['map_nll_bits']/T for r in maps])) if maps else None,'mean_legal_mass_bps':float(np.mean([r['legal_mass_bits']/T for r in maps])) if maps else None,'mean_map_mass_gap_bps':float(np.mean([(r['map_nll_bits']-r['legal_mass_bits'])/T for r in maps])) if maps else None,'rows':rows}
...
def main(paths):
 ...
 try:
  ...
  out={'protocol':'CONDITIONAL / STREAMING SIDE-INFORMATION GATE — NOT a standalone-file SOTA claim','previous_records':2,'target_shape':[C,T],'c0':C0,'t0':T0,'samples':C*T,'global_std':std,'eps':eps,'sz3':{'bytes':int(szb),'bps':8*szb/(C*T),'orientation':ori},'frozen_standalone_hybrid_bytes':FROZEN_STANDALONE_HYBRID,'rows':rows,'best':best,'scope':'Constructive version of PR242 random-codebook mass. Two previous Imperial records are decoder-known side information and define independently per channel the old first-order lattice Markov generator. The unchanged hard-error box defines all legal states. Viterbi chooses the highest-probability complete legal path; a real 64-bit arithmetic stream transmits that path under the same integer-smoothed Markov law. Decoder uses only the same previous records plus the physical stream, reconstructs every q and passes source hard error. -log(total legal probability mass) remains diagnostic only; achieved bytes are the MAP path container. Missing-support channels fall back to explicit Zstd int32 lattice paths. This is conditional/streaming evidence only; standalone promotion requires self-bootstrap or full side-information accounting.'}
  json.dump(out,open('imperial_markov_map_legal_codec.json','w'),indent=2)
  print(json.dumps({'summary':{'best_bytes':best['bytes'],'best_h':best['h_over_eps_approx'],'best_bps':best['bps'],'sz3_bytes':int(szb),'frozen_standalone_hybrid':FROZEN_STANDALONE_HYBRID,'gain_vs_sz3':int(szb)/best['bytes'],'gain_vs_standalone_hybrid':FROZEN_STANDALONE_HYBRID/best['bytes'],'map_channels':best['map_channels'],'fallback_channels':best['fallback_channels'],'map_nll_bps':best['mean_map_nll_bps'],'legal_mass_bps_diagnostic':best['mean_legal_mass_bps'],'map_mass_gap_bps':best['mean_map_mass_gap_bps']}},indent=2),flush=True)
 finally:
  for f in fs:f.close()
