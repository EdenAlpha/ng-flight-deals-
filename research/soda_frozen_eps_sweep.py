...
def main(path,frac):
    X,gx,gy,dt=load(path);std=float(X.astype(np.float64).std());eps=float(frac)*std;step=2*eps;raw=int(X.nbytes)
    ...
    me=float(np.max(np.abs(X-Y)));container=TOPS+len(mb)+int(bo[0]);szb,sze=sz3_bytes(X,eps)
    out={'file':os.path.basename(path),'shape':list(X.shape),'std':std,'epsilon_fraction_of_std':float(frac),'eps':eps,'step':step,'raw_bytes':raw,'geometry':geom,'K_nonzero_fraction':float(np.mean(K!=0)),'main_bytes':len(mb),'main_parts':parts,'outlier_bytes':int(bo[0]),'top_header_bytes':TOPS,'container_bytes':container,'ratio':float(raw/container),'maxerr':me,'valid':bool(me<=eps*(1+3e-6)),'sz3':{'bytes':int(szb),'ratio':float(raw/szb),'maxerr':float(sze)},'gain_vs_direct_sz3':float(szb/container),'frozen_definition':{'order':list(FROZEN_ORDER),'context_mode':FROZEN_CTXMODE,'backend_method_ids':list(FROZEN_METHODS)}}
    if not out['valid']:raise RuntimeError(('hard error',me,eps))
    print(json.dumps({'frac':frac,'eps':eps,'bytes':container,'ratio':out['ratio'],'sz3_bytes':int(szb),'gain_sz3':out['gain_vs_direct_sz3'],'K_nonzero_fraction':out['K_nonzero_fraction'],'maxerr':me},indent=2),flush=True)
