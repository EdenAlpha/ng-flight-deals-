...
CASES=[(2,'spatial'),(2,'temporal'),(2,'median'),(4,'spatial'),(4,'temporal'),(4,'median'),(8,'spatial'),(8,'temporal'),(8,'median')]
def main(path):
 with h5py.File(path,'r') as f:
  ...
  for name,t0,c0 in specs:
   ...
   for M,side in CASES:
    r=run(X,b,1,True,1.0,M,side);r.update(tile=name,sz3_bytes=sb,gain_vs_sz3=sb/r['bytes'],bps=8*r['bytes']/X.size);rows.append(r)
  ...
  combos.sort(key=lambda x:x['bytes']);out={'std':std,'eps':eps,'fixed_parent_phase':1,'fixed_parent_demod':True,'fixed_parent_weight':1.0,'tiles':[x[0] for x in specs],'combos':combos,'rows':rows,'scope':'Corrected fast directional gate for PR247 centered on the actual PR236 winner: Nyquist demod true, temporal weight1.0, phase1. M2/4/8 and spatial/temporal/median side views on early/center/edge tiles.'};print(json.dumps(out,indent=2),flush=True);json.dump(out,open('imperial_dualview_truewinner_gate.json','w'),indent=2)
