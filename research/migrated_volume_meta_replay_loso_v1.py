...
def meta_fit(train_tiles,seed):
    ...
    # Build three source-survey episodes. Split each tile by time so inner support
    # and meta query are disjoint in that source survey.
    episodes=[]
    for X,eps in train_tiles:
        A,T,I,R=q.b.build(q.crop(X),eps);A=(A-mu)/sd;Y=q.b.cls(T)
        mid=int(np.median(I[:,2]))
        sm=I[:,2] <= mid; qm=I[:,2] > mid
    ...
    for me in range(META_EPOCHS):
        ...
        for ei in order:
            SX,SY,QX,QY=episodes[ei]
            fast=copy.deepcopy(net);fast.train()
            inner=torch.optim.SGD(fast.parameters(),lr=INNER_LR)
            # Fixed deterministic mini-batch traversal; support is already from a
            # source training survey, never the held-out target.
            ...
            qloss=F.cross_entropy(fast(QX),QY);qloss.backward();losses.append(float(qloss.detach()))
            # First-order MAML: map query gradient at adapted weights back to the
            # shared initialization, omitting second-order inner-step terms.
...
def main(a):
    # fr.fit_base will call fr._orig_fit then freeze the pristine state used for
    # base scoring. Replace only that underlying initializer with meta_fit.
    fr._orig_fit=meta_fit
    fr.main(a)
    out=json.load(open(a.out));out['kind']='four-survey-meta-initialized-causal-full-replay-loso-v1';out['meta_training_uses_heldout_survey']=False;out['meta_epochs']=META_EPOCHS;out['meta_inner_steps']=INNER_STEPS;out['meta_inner_lr']=INNER_LR;out['meta_lr']=META_LR;out['meta_support_cap']=SUPPORT_CAP;out['meta_query_cap']=QUERY_CAP;out['meta_rule']='source-survey temporal support/query episodes; first-order MAML initialization; same rule for every LOSO split';out['note']='Meta-initialized full causal replay diagnostic. Held-out target never participates in base or meta fitting. Every target chunk is scored before target adaptation. Ideal probability rate only.'
