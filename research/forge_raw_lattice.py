...
def orders(gx,gy,sx,sy,offs):
    ...
    add('file',idx)
    add('x_y',np.lexsort((idx,gy,gx)))
    add('y_x',np.lexsort((idx,gx,gy)))
    ...
    add('pca_line',np.lexsort((idx,a,b)))
    add('pca_along',np.lexsort((idx,b,a)))
    rs=np.sqrt((gx-sx)**2+(gy-sy)**2);add('radial',np.lexsort((idx,rs)))
    add('offset_header',np.lexsort((idx,offs)))
...
# Dense controls only on file order and best geometry-like orders, to avoid excessive runtime.
...
# Fair SZ3: identical epsilon and same deterministic receiver reorderings.
szrows=[]
