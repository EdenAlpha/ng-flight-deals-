...
# ---------- deterministic stochastic codebook ----------
@njit(cache=True)
def _next_u64(x):
...
def _candidate(idx,seedbase,mode,prev,prev2,left,leftprev,hasleft,vals,cum,total):
 # Split index into a stable nonzero 64-bit state; codebook is regenerated identically at decode.
...
# ---------- prefix-trained decoder-known probability model ----------
def predictor(mode,prev,prev2,left,leftprev,hasleft):
 if not hasleft:
  return 2*prev-prev2 if mode in (3,4) else prev
 if mode==0:return prev
 if mode==1:return (prev+left)//2
 if mode==2:return prev+(left-leftprev)
 if mode==3:return 2*prev-prev2
...
# ---------- actual adaptive Rice index stream ----------
class BW:
...
def seedbase(region_id,c,block):
 # Values are public coordinates, not source-trained side information.
...
def main(path):
 with h5py.File(path,'r') as f:
  ...
  for rid,(region,c0) in enumerate(REGIONS):
   ...
   # Prefix is an ordinary exact integer stream; decode it before constructing the shared model.
   ...
   # Encode channels sequentially. Left-channel accepted Q is therefore decoder-known.
   QE=np.zeros_like(Q);QE[:,:TRAIN]=Qpd;bw=BW();fallback=[];indices=[];escapes=0;score=12*256
   per_ch=[]
   for c in range(C):
    hits=[];esc=0
   ...
   # Full byte decoder: exact prefix, Rice indices, compressed escapes, and deterministic codebook replay.
   ...
   for c in range(C):
    for bno,t0 in enumerate(range(TRAIN,NT,L)):
     ...
     if idx==MAX_TRIES:
      if fp+L>len(fbd):raise RuntimeError((region,'fallback eof'));q=fbd[fp:fp+L];fp+=L
     elif idx<MAX_TRIES:
   ...
   # Matched SZ3 on identical samples; each returned stream is independently decoded inside szrun.
   sz=0
   for t0 in range(0,NT,TB):n,_=m.szrun(X[:,t0:min(t0+TB,NT)],eps);sz+=int(n)
   ns=X.size;held=(NT-TRAIN)*C;succ=1-escapes/(((NT-TRAIN)//L)*C)
   row={'region':region,'c0':c0,'shape':[C,NT],'samples':int(ns),'eps':eps,'lattice_h':h,'block_length':L,'max_tries':MAX_TRIES,
        ...
        'median_index':float(np.median([x for x in indices if x<MAX_TRIES])) if escapes<len(indices) else None,'mean_index':float(np.mean([x for x in indices if x<MAX_TRIES])) if escapes<len(indices) else None,
  ...
  out={'global_std':gstd,'eps':eps,'rows':rows,'scope':'Constructive decoder-real random-covering/set-code pilot. No previous record is side information. The first 1024 samples of each four-channel region are nearest-q encoded exactly through encode_k and decoded; that decoded prefix deterministically selects one fixed causal predictor and builds its residual histogram. For every subsequent nonoverlapping 8-sample block, encoder and decoder share a deterministic stochastic codebook sampled from that prefix model using only public coordinates, already-decoded same-channel history and already-decoded left-channel Q. Encoder searches for the first generated candidate lying coordinatewise inside the source hard-error intervals and transmits only its integer codebook index using a decoder-synchronized adaptive Rice code. The decoder regenerates that whole block from the index without knowing the source intervals. Search failure at 2^18 candidates is a real escape: the nearest legal Q block is appended to a Zstd-compressed int16 escape stream. Prefix, Rice payload, escape payload and 64 framing bytes are all charged; the complete container is parsed, every index replayed, Q reproduced exactly, and the final source max error checked. Matched SZ3 is rerun on identical samples. This is a constructive short-block approximation to probability-mass/set coding, not an entropy oracle. No AI. Draft/do not merge.'}
