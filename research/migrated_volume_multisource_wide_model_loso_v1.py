#!/usr/bin/env python3
"""Nine-source-tile LOSO with a wider still-small probability model.

Same data, held-out targets, normalization rule, causal replay and eight source
training epochs as the converged nine-tile gate. Only model capacity changes:
160/112/64 -> 320/240/160 hidden units. Target chunks are always charged before
adaptation. Ideal probability-rate diagnostic only; not a serialized codec.
"""
...
SOURCE_FRACS=(.15,.50,.85);TARGET_FRACS=(.15,.50,.85);MAX_TRAIN=720000;EPOCHS=8;HIDDEN=(320,240,160);PROTOCOL='multisource-9tile-wide-model-full-replay-v1'

def big_fit(train_tiles,seed):
    AA=[];TT=[];RR=[]
    ...
    A,T=q.deterministic_subsample(A,T,MAX_TRAIN);mu=A.mean(0);sd=A.std(0);sd[sd<.1]=1.;A=(A-mu)/sd;Y=q.b.cls(T)
    ...
    torch.manual_seed(seed);np.random.seed(seed);net=M(A.shape[1]);opt=torch.optim.AdamW(net.parameters(),lr=1.5e-3,weight_decay=3e-4);Xt=torch.from_numpy(A);Yt=torch.from_numpy(Y);idx=torch.arange(len(Xt));bs=8192
    for ep in range(EPOCHS):
        ...
        print('WIDE_BASE_FIT',seed,ep,tot/len(Xt),flush=True)
    net.eval()
    for p in net.parameters():p.requires_grad_(False)
    print('WIDE_MODEL_PARAMS',sum(p.numel() for p in net.parameters()),flush=True);return net,mu,sd,static

def main(a):
    fr._orig_fit=big_fit
    m=json.load(open(a.manifest));e=json.load(open(a.eps));raw={}
    for ds in q.SURVEYS:
        raw[ds]={}
        for frac in SOURCE_FRACS:
            X,ep,md=q.b.extract(ds,m,e,frac);raw[ds][frac]=(q.crop(X),ep,md);print('WIDE_MULTI_RAW',ds,frac,q.crop(X).shape,flush=True)
    targets=[]
    for ti,target in enumerate(q.SURVEYS):
        ...
        net,mu,sd,static=fr.fit_base(train,20261019+ti);state,u1,u2=fr.new_adapter(95000+ti);reservoir=[];base_rows=[];adapt_rows=[]
        for pi,frac in enumerate(TARGET_FRACS):
            ...
            row=fr.score_and_adapt(net,mu,sd,static,X,ep,meta,state,u1,u2,reservoir,pi==0);adapt_rows.append(row);print('WIDE_MULTI_REPLAY',json.dumps(row),flush=True)
    ...
    out={'kind':'four-survey-multisource-wide-model-full-replay-loso-v1','status':'diagnostic_ideal_probability_rate_not_serialized_codec','protocol':PROTOCOL,'hidden':list(HIDDEN),'source_tiles_per_split':9,'source_training_epochs':EPOCHS,'max_train_examples':MAX_TRAIN,'held_out_survey_used_in_base_training':False,'test_positions_all_charged':True,'targets':targets,'base_overall_byte_weighted_gain_vs_sz3':float(sum(z['sz3_bytes'] for z in allb)/sum(z['ideal_bytes_plus_header'] for z in allb)),'full_replay_overall_byte_weighted_gain_vs_sz3':float(sum(z['sz3_bytes'] for z in alla)/sum(z['ideal_bytes_plus_header'] for z in alla)),'all_full_replay_positions_beat_sz3':bool(all(z['gain_vs_sz3_ideal']>=1 for z in alla)),'note':'Capacity-only scaling relative to converged 9-tile small model. Target remains fully held out. Ideal rate only.'}
    Path(a.out).write_text(json.dumps(out,indent=2));print('WIDE_MODEL_FINAL',json.dumps({k:v for k,v in out.items() if k!='targets'},indent=2),flush=True)
