#!/usr/bin/env python3
"""Monolithic v4 context-arithmetic scale test on large native migrated blocks."""
...
def compete(X,eps):
 ...
 for tid in TIDS:
  ...
  if e>eps*(1+3e-6):raise RuntimeError(('hard',tid,e,eps))
  rows.append({'name':c.NAMES[int(tid)],'bytes':len(b)+OVERHEAD,'maxerr':e})
 for mod,name in ((br,'fast_delta_brotli_multitraversal'),(fh3,fh3.NAME)):
  q=mod.candidate(X,eps);rows.append({'name':name,'bytes':int(q['bytes'])+OVERHEAD,'maxerr':float(q['maxerr'])})
...
def stream(objects,tag,eps):
 ...
 try:
  ...
  for wi,frac in enumerate(large.FRACTIONS):
   ...
   row={'window_index':wi,'window_fraction':frac,'shape':list(map(int,X.shape)),'samples':int(X.size),'winner':best['name'],'ours_bytes':int(best['bytes']),'sz3_bytes':int(sb),'gain_vs_sz3':gain,'maxerr':best['maxerr'],'sz3_maxerr':float(sme),'candidate_bytes':{q['name']:q['bytes'] for q in cands}}
   rows.append(row);print('MVCTX4LARGE',tag,wi,X.shape,sb,best['bytes'],gain,best['name'],flush=True)
...
def run(a):
 ...
 out={'kind':'migrated-volume-large-context-v4','compression_class':'migrated_poststack_3d','dataset_id':a.dataset,'epsilon':eps,'positions':len(rows),'wins':sum(q['gain_vs_sz3']>1 for q in rows),'min_gain':min(q['gain_vs_sz3'] for q in rows),'median_gain':float(np.median([q['gain_vs_sz3'] for q in rows])),'byte_weighted_gain':sum(q['sz3_bytes'] for q in rows)/sum(q['ours_bytes'] for q in rows),'all_valid':True,'streams':ss}
