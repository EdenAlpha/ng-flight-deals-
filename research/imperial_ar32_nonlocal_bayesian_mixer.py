...
def main(path):
    ...
    with h5py.File(path,'r') as f:
        ...
        json.dump({'global_std':gstd,'eps':eps,'rows':rows,'scope':'Stack of the two strongest independently positive hard-region mechanisms: the decoder-real prefix-trained bounded nonlocal residual stencil and the non-neural Bayesian probability mixer. The nonlocal K stream is selected only by the incumbent exact-backend screen, then encoded from scratch by the same four-expert mixer tested on baseline K. Nonlocal tap IDs/float32 coefficients/framing are fully charged. Exact mixed-arithmetic K decoding, nonlocal source replay and unchanged max error are mandatory. Baseline AR32, baseline mixer, nonlocal ordinary arithmetic, combined nonlocal+mixer and matched SZ3 are rerun on hard/easy. No AI. Draft/do not merge.'},open('imperial_ar32_nonlocal_bayesian_mixer.json','w'),indent=2)
