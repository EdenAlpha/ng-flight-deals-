...
def _pack_int(vals):
    ...
    return b'I'+bytes([mode])+vi(len(bits))+bits+(absb if mode==0 else deltab)
...
def _sample_group_score(vals,cols,ks,limit=12000):
    state={};stream=bytearray();seen=0;step=max(1,len(vals)//limit)
...
def pack(data):
    ...
    for j,c in enumerate(cols):
        cand=[_choose(c)];ksall=_keysets(j);fdc=[]
        for ks in ksall:
            if _sample_fd_ok(c,cols,ks):fdc.append(ks)
        for ks in fdc[:6]:
