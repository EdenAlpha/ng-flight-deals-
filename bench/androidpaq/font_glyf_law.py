...
def pack(d):
 try:
  ...
  e=f.reader.tables['glyf'];g=WOFF2GlyfTable('glyf');g.glyphs=f['glyf'].glyphs;g.glyphOrder=f.getGlyphOrder();td=g.transform(f)
...
def unpack(rep):
 ...
 f=TTFont(BytesIO(bytes(sk)),lazy=True,recalcTimestamp=False);g=WOFF2GlyfTable('glyf');g.reconstruct(td,f);raw=g.compile(f)
 if len(raw)!=ln:raise ValueError('glyf length mismatch')
 sk[off:off+ln]=raw;return bytes(sk)
