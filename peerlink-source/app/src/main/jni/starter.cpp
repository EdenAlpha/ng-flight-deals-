...
#define PACKAGE_NAME "com.peerlink.app"
#define SERVER_NAME "peerlink_prime"
#define SERVER_CLASS_PATH "com.peerlink.app.godmode.PrimeServerMain"
...
static void run_server(const char *dex_path, const char *main_class, const char *process_name, const char *auth_token) {
...
#define ARG_PUSH_DEBUG_ONLY(v, arg)
    ...
    if (auth_token != nullptr && auth_token[0] != '\0') {
        ARG_PUSH_FMT(argv, "--token=%s", auth_token)
    }
...
static void start_server(const char *path, const char *main_class, const char *process_name, const char *auth_token) {
    pid_t pid = fork();
    switch (pid) {
        case -1: {
...
int main(int argc, char *argv[]) {
    ...
    if (auth_token.size() < 64) {
        perrorf("fatal: missing PrimeServer auth token.\n");
        exit(EXIT_FATAL_APP_PROCESS);
    }
    ...
    fflush(stdout);
    LOGD("start_server");
    start_server(apk_path.c_str(), SERVER_CLASS_PATH, SERVER_NAME, auth_token.c_str());
}
